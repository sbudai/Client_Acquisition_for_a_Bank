...
keys <- data.table(import('~/Documents/creds.csv'))
...
export(owner_company_list, file_out)
...
export(branch_zip_list, file_out)
...
initial summary of company list
```{r, warning=FALSE, include=TRUE}
...
let's drop commas from Szekhely and SzekhelyVaros columns
```{r, warning=FALSE, include=TRUE}
...
# dropping plotly package and relaunch rio package
...
downloading postcode list of all Hungarian settlements
```{r, warning=FALSE, include=TRUE}

# downloading & decompressing into a corresponding folder
temp <- tempfile()
...
postcode_list <- data.table(import(paste(getwd(), '/_tmp/postcode_list.csv', sep = '')))
...
downloading those companies list which employed people without registration
```{r, warning=FALSE, include=TRUE}

# this link is changing weekly / you can check the valid one on http://nav.gov.hu/nav/adatbazisok/benemjelentett
unreg_emp <- data.table(import('http://nav.gov.hu/data/cms328145/Be_nem_jelentett_alkalmazottat_foglalkoztatok_20170329.xlsx'))

# changing accented characters into non-accented ones in the downloaded data
colnames(unreg_emp) <- gsub('(', '_', gsub(')', '', gsub(' ', '', stri_trans_totitle(gsub('_', ' ', stri_trans_general(colnames(unreg_emp), 'Latin-ASCII'))))), fixed = TRUE)
...
creating a new, modified instance of company list 
```{r, warning=FALSE, include=TRUE}

# importing
file_in = paste(getwd(), '/_tmp/company_list.RData', sep = '')
company_list_1 <- data.table(import(file_in))
...
let's denote those companies which used unregistered employees
```{r, warning=FALSE, include=TRUE}
...
joining latitude and longitude data of postcodes to company list
```{r, warning=FALSE, include=TRUE}
...
joining latitude and longitude data of postcodes to branch postcode list
```{r, warning=FALSE, include=TRUE}
...
export(branch_postcode_list, file_out)
...
export(company_list_1, file_out)
...
export(owner_company_list_1, file_out)
...
export(company_list_2, file_out)
...
export(company_list_3, file_out)
...
company_list_3 <- data.table(import(paste(getwd(), '/_tmp/company_list_3.RData', sep = '')))
...
dt <- copy(data.table(h2o.performance(model, dh2o_train)@metrics$thresholds_and_metric_scores[18:19]))

export(dt2, '~/Desktop/h2o_model_performance.csv')

p26 <- ggplot(dt, aes(x = fpr, y = tpr)) +
